...
void edrawstring(GLfloat x, GLfloat y, errno_t* string)
{
    errno_t *c;
    glRasterPos2f(x, y);
    ...
    for (j = 1; j <= n; j++)
    {
        for (k = 1; k <= n; k++)
        {
            if (cost[j][k] != 999 && j < k)
            {
                ...
                glEnd();
                errno_t x;
                x = _itoa_s(cost[j][k], s, 10);
                s1 = &x;
                edrawstring((x1 + x2 - 16) / 2, (y1 + y2 + 22) / 2, s1);
                glFlush();
            }
            ...
            if (cost[j][k] != cost[k][j] && cost[j][k] != 999 && j > k)
            {
                ...
                x = _itoa_s(cost[j][k], s, 10);
                s1 = &x;
                edrawstring((x1 + x2 + 20) / 2, (y1 + y2 + 36) / 2, s1);
                glFlush();
            }
        }
    }
...
void shortestpath(int src)
{
    ...
    for (int x = 0; x < l; x++)
    {
        ...
        if (p < q)
        {
            ...
            glEnd();
            errno_t x;
            x = _itoa_s(cost[p][q], s, 10);
            s1 = &x;
            edrawstring((x1 + x2) / 2, (y1 + y2 + 500) / 2, s1);
        }
